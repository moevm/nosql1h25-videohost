...
const subscribe = async (userId: string) => {
  ...
  if (!response.ok) return error('Подписка', 'Не удалось подписаться')
...
const unsubscribe = async (userId: string) => {
  ...
  if (!reponse.ok) return error('Отписка', 'Не удалось отписаться')
...
const showParams = async () => {
  ...
  const baseURL = `${import.meta.env.VITE_API}search/users?` +
    new URLSearchParams({
      registeredAfter,
      registeredBefore,
